...
window.openSnesEmulator = async function() {
    ...
    if (window._snesListener) {
        window.removeEventListener('message', window._snesListener);
        window._snesListener = null;
    }
    ...
    output.innerHTML = `
        <style>
            #snes-display-wrapper:fullscreen {
                ...
                border: none !important; background: black !important;
                display: flex !important; align-items: center !important; justify-content: center !important;
            }
            #snes-display-wrapper:fullscreen iframe { width: 100% !important; height: 100% !important; border: none !important; }
        </style>
        <div class="nes-terminal-wrapper">
            ...
            <div style="display:flex; justify-content:center; margin-bottom:10px;">
                <select id="snes-rom-select" class="nes-model-selector" style="flex-grow:1; max-width:300px; margin-right: 10px;">
            ...
            <div id="snes-display-wrapper" class="nes-screen-container" style="
                position: relative; border: 1px solid #0f0; width: 100%; aspect-ratio: 4 / 3;
                background: black url('${chrome.runtime.getURL("Emulators/snes/cover.jpg")}') no-repeat center center; background-size: cover;
            ">
            ...
            <div id="snes-debug-log" style="color:#0f0; font-family:'Courier New'; font-size:12px; margin-top:5px; height:20px; text-align:center;"></div>
            <div style="display: block; width: 100%; box-sizing: border-box; margin: 5px auto 0 auto; font-size:0.65rem; opacity:0.7; font-family: 'Courier New', monospace; text-align:center; line-height:1.5;"><span style="color:#fff;">CONTROLS:</span> WASD = MOVE | START = <span style="color:#fff;">ENTER</span> | SELECT = <span style="color:#fff;">R-SHIFT</span><br>A=<span style="color:#fff;">Z</span> B=<span style="color:#fff;">X</span> X=<span style="color:#fff;">C</span> Y=<span style="color:#fff;">V</span> | L=<span style="color:#fff;">L-SHIFT</span> R=<span style="color:#fff;">PG-DN</span><br><span style="color:#fff;">1</span> = SAVE STATE | <span style="color:#fff;">4</span> = LOAD STATE</div>
        </div>`;
    ...
    slotSelect.addEventListener('change', (e) => {
        ...
        if (savedStateBase64) {
            ...
            debugLog.innerHTML += ` <span style="color:#aaa;">(Found Saved Data)</span>`;
        } else {
            debugLog.innerHTML += ` <span style="color:#aaa;">(Empty)</span>`;
        }
    ...
    fullBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        if (!document.fullscreenElement) displayWrapper.requestFullscreen().catch(() => {});
    ...
    muteBtn.addEventListener('click', (e) => {
        ...
        if (iframe) {
            ...
            muteBtn.style.color = isMuted ? '#0f0' : 'red';
        }
        muteBtn.blur();
    });

    const ROMS = [
        "mainBankZero_llvm-mos - demo.smc",
    ...
    async function findRomUrl(baseName) {
        ...
        return chrome.runtime.getURL(root + baseName);
    }
        ...
        try {
            ...
            iframe.onload = () => {
                iframe.contentWindow.postMessage({ 
                    ...
                    retroarchConfig: snesRetroarchConfig 
                }, '*');
    ...
    window._snesListener = async function(e) {
        if (e.data.status === 'running') {
            debugLog.innerHTML = `<span style="color:#0f0;">SYSTEM ONLINE</span>`;
        }
        ...
        else if (e.data.command === 'export_state') {
            console.log(`[SNES PARENT] Received Export Data: ${e.data.data.byteLength} bytes`);
            ...
            try {
                ...
                setTimeout(() => { debugLog.innerHTML = `<span style="color:#0f0;">SYSTEM ONLINE</span>`; }, 2000);
            } catch(err) {
                 console.error("Storage Error:", err);
                 debugLog.innerHTML = `<span style="color:red;">SAVE FAILED (STORAGE LIMIT?)</span>`;
            }
        }
    };
    window.addEventListener('message', window._snesListener);
};