// Games/matrixoverloaded/matrixoverloaded_launcher.js
...
const keys = {
    ...
    attack:{ code: 'KeyA',       key: 'a',         keyCode: 65, pressed: false }, 
    pickup:{ code: 'KeyS',       key: 's',         keyCode: 83, pressed: false }  
};
...
function updateGamepad() {
    ...
    const gp = gamepads[0]; 
    if (gp) {
        // 1. ANALOG & D-PAD MOVEMENT (Including Y-Axis for Up/Down)
        ...
        // 2. ACTION BUTTONS
        const isJumpInput = gp.buttons[0]?.pressed; // A Button
        const isAttackInput = gp.buttons[1]?.pressed || gp.buttons[2]?.pressed; // B or X
        const isPickupInput = gp.buttons[3]?.pressed; // Y Button

        // Apply Vertical Movement
        if (isUp && !keys.up.pressed) { 
            keys.up.pressed = true; 
            triggerKeyEvent('keydown', keys.up); 
        } else if (!isUp && keys.up.pressed) { 
            keys.up.pressed = false; 
            triggerKeyEvent('keyup', keys.up); 
        }

        if (isDown && !keys.down.pressed) { 
            keys.down.pressed = true; 
            triggerKeyEvent('keydown', keys.down); 
        } else if (!isDown && keys.down.pressed) { 
            keys.down.pressed = false; 
            triggerKeyEvent('keyup', keys.down); 
        }

        // Apply Horizontal Movement
        if (isLeft && !keys.left.pressed) { 
            keys.left.pressed = true; 
            triggerKeyEvent('keydown', keys.left); 
        } else if (!isLeft && keys.left.pressed) { 
            keys.left.pressed = false; 
            triggerKeyEvent('keyup', keys.left); 
        }

        if (isRight && !keys.right.pressed) { 
            keys.right.pressed = true; 
            triggerKeyEvent('keydown', keys.right); 
        } else if (!isRight && keys.right.pressed) { 
            keys.right.pressed = false; 
            triggerKeyEvent('keyup', keys.right); 
        }
